...
Every cGLP run is safe, so correctness (Definition~\ref{def:ts}) reduces to liveness: a cGLP run is \temph{correct} iff for every equivalence class $[t] \in \calT/{\sim}$, whenever $[t]$ is enabled some $t' \in [t]$ is eventually taken.
\end{definition}
