...
We present GLP syntax,  operational semantics,  programming examples, and conclude with the grassroots social graph---the foundational platform on which other grassroots platforms are built.
...
\label{ex:merge}
Consider the quintessential concurrent logic program for fairly merging two streams, written in GLP:\footnote{Mode annotations (\texttt{?} suffix) and type declarations (\texttt{procedure ...}, \texttt{T ::= ...}) used informally in examples are formally introduced in the Typed GLP companion paper~\cite{shapiro2026types}.}
\begin{verbatim}
Stream ::= [] ;[_|Stream].
...
For cGLP, $t_1 \sim t_2$ if:
\begin{itemize}
\item both are Reduce transitions that reduce the same goal (by identity) with the same clause, or
\item both are Communicate transitions that apply the counterpart of the same writer assignment to the same goal (by identity).
\end{itemize}
\end{definition}
...
The core of the program is the procedure \verb|agent/4|; three of its clauses follow. The user's \verb|connect(Target)| request triggers a cold-call: the agent sends \verb|intro(Id, Resp)| on the network and injects the paired reader \verb|Resp?| into its own user-input stream, so the eventual response surfaces locally as a normal user-side message:
\begin{verbatim}
...
A network switch process routes messages between agents in simulation; in deployment, it is replaced by maGLP's Cold-call transaction (Section~\ref{sec:maglp}). The detailed walkthrough with figures, the complete program (helper predicates, actor implementations, and boot variants), and a comprehensive Alice/Bob/Charlie scenario appear in \appref{appendix:social-graph-walkthrough}{\arxivref} and \appref{appendix:social-graph-complete}{\arxivref}.
