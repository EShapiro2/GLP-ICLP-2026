...
Despite these differences, cGLP adopts the same notions of proper run and successful run as LP\ifappendix{} (Definition~\ref{def:proper-run}, Appendix~\ref{appendix:lp})\fi, and has the same notion of logical consequence as LP. The cGLP outcome differs from LP's: $\sigma_n$ accumulates only readers counterparts and so leaves the writers of $G_0$ unbound, which the writers counterpart restores. Let $/?$ be an operator that replaces every reader by its paired writer.

\begin{restatable}[cGLP Computation is Deduction]{proposition}{propGLPDeduction}
\label{prop:cglp-deduction}
Let $(G_0$ \texttt{:-} $G_n)\sigma_n^\star$ be the outcome of a proper cGLP run $\rho: (G_0,\sigma_0) \rightarrow \cdots \rightarrow (G_n, \sigma_n)$ of $cGLP$, where $\sigma_n^\star$ is $\sigma_n \cup \sigma_n!$ applied to a fixed point. Then $(G_0$ \texttt{:-} $G_n)\sigma_n^\star/?$ is a logical consequence of $P/?$.
\end{restatable}
...
Guard occurrences count toward SRSW satisfaction: a guard reader $X?$ counts as the paired-reader occurrence SRSW requires, so a variable whose writer $X$ occurs in the head and whose reader $X?$ occurs in a guard satisfies SRSW with one occurrence of each.
%
