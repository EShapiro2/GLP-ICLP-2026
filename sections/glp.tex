...
On the one hand, GLP has no unification, only pattern-matching and single-writer variables.  On the other hand, goal reduction may write on several variables atomically.  The result is that the entire range of concurrent logic programming technique is available in GLP, including streams and their merging and distribution, messages with reply variables, network reconfiguration, and the whole range of meta-interpreters: fail-safe, termination-detecting, computation control, tracing and debugging.  The  one exception is the use of logic variables for mutual exclusion --- this functionality has to be achieved via a monitor---the standard distributed-computing solution.
