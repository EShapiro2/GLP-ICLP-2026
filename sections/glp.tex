...
\begin{definition}[Transition System, Computation, Run, Safe, Live, Correct (cf.~\cite{shapiro2021multiagent,lewis2026volitional})]
\label{def:ts}
...
GLP \emph{system predicates} such as \verb|:=| (arithmetic assignment), \verb|now| (clock access), \verb|=..| (term composition/decomposition), and \verb|random| (seeded random-number generation) are defined by GLP clauses whose bodies may invoke \emph{body kernel predicates}---runtime-implemented primitives not directly accessible to user programs. The system predicate's own guards ensure that body kernel preconditions are met before invocation. For example, arithmetic assignment is defined recursively as follows:
