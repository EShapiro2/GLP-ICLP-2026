...
%------------------------------------------------------------------------------
\subsection{Ending Friendship}
%------------------------------------------------------------------------------

\textcolor{red}{Once two agents are connected as friends, either agent may initiate a unilateral termination of the relationship. Upon initiation, the initiating agent immediately removes the other agent from its friend list. The counterpart agent subsequently updates its own friend list by removing the initiator. }

(Figure~\ref{fig:end-friendship}).

\begin{figure*}[t]
\begin{multicols}{2}
\begin{verbatim}
%%look up friend, send unfriend message
with Done variable
procedure send_unfriend(_?, _?,
FriendList?, FriendList, _).
send_unfriend(Id, Target, Fs,Fs1?,
Done?) :-
    lookup(Target?, Fs?, Result),
    do_send_unfriend(Result?, Id?,
    Target?, Fs?, Fs1, Done).

procedure do_send_unfriend(
LookupResult?,_?, _?,
FriendList?, FriendList, _).
do_send_unfriend(found(Out), Id,
Target, Fs, Fs1?, Done?) :-
    update(Target?, [unfriend(Id?,
    Done)|Out?], Fs?, Fs1).
\end{verbatim}
\columnbreak
\begin{verbatim}
%%notify user, wait for Done, then close
and remove, 
procedure await_unfriend(_?, _?, _?, 
MsgStream?, FriendList?).
await_unfriend(Done, Id, Target, In, Fs) :-
    Done? =?= true |
    close_and_remove(Target?, Fs?, Fs1),
    lookup_send(user, msg(agent, user,
    unfriended(Target?)),Fs1?, Fs2, _),
    social_graph(Id?, In?, Fs2?).
\end{verbatim}
\begin{verbatim}
%%close stream and remove entry
procedure close_and_remove(_?, FriendList?,
FriendList).
close_and_remove(Key, [(K, S)|Rest], Rest?)
:- Key? =?= K? | S = [].
close_and_remove(Key, [(K, V)|Rest], 
[(K?, V?)|Rest1?]) :-
    otherwise | close_and_remove(Key?
    Rest?, Rest1).
close_and_remove(_, [], []).
\end{verbatim}
\end{multicols}
\caption{Agent unfriend mechanism}
\label{fig:end-friendship}
\end{figure*}
%------------------------------------------------------------------------------
