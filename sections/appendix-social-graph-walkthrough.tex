...
\begin{multicols}{2}
\begin{verbatim}
procedure remove_output(OutputKey?, 
OutputsList?, OutputsList).
remove_output(Key, [output(K, [])|Rest],
Rest?) :-
    Key? =?= K? | true.
...
agent(Id, [msg('_user', Id1, unfriend
(Target))|UserIn], NetIn, Outs) :-
    Id? =?= Id1?, ground(Target?) |
    lookup_send(friend(Target?), msg(Id?,
    Target?, unfriend(Id?)), Outs?, Outs1),
    remove_output(friend(Target?), Outs1?,
    Outs2),
    lookup_send('_user', msg(agent, '_user',
    unfriended(Target?)), Outs2?, Outs3),
...
unfriend(_))|NetIn], Outs) :-
    Id? =?= Id1?, ground(From?) |
    remove_output(friend(From?), Outs?,
    Outs1),
    lookup_send('_user', msg(agent, '_user', 
    unfriended(From?)), Outs1?, Outs2),
