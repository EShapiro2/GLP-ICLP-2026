...
\item \verb|no_readers(X)| succeeds if \verb|X| contains no readers; suspends if \verb|X| contains any readers; never fails. Useful for ensuring a term is safe for external output (ground terms or writers only).
...
\verb|ground/1|, \verb|no_readers/1|, and \verb|known/1| are monotonic. \verb|ground(X)| implies both \verb|no_readers(X)| and \verb|known(X)|, which do not imply each other: \verb|no_readers(X)| succeeds but \verb|known(X)| fails; \verb|no_readers(f(X?))| suspends but \verb|known(f(X?))| succeeds.
