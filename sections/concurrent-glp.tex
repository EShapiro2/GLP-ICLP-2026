...
We use set notation to denote a multiset of unit goals; the same goal may occur more than once in a body.

\begin{example}[Fair Merge]
\label{ex:merge}
Consider the quintessential concurrent logic program for fairly merging two streams, written in GLP:\footnote{Mode annotations (\texttt{?} suffix) and type declarations (\texttt{procedure ...}, \texttt{T ::= ...}) used informally in examples are formally introduced in the TGLP companion paper~\cite{shapiro2026types}.}
\begin{verbatim}
Stream ::= [] ;[_|Stream].
...
%------------------------------------------------------------------------------
\subsection{cGLP Operational Semantics}
...
Despite these differences, cGLP adopts the same notions of proper run, successful run, and outcome as LP (Definition~\ref{def:proper-run}, \ref{appendix:lp}), and has the same notion of logical consequence as LP. Let $/?$ be an operator that replaces every reader by its paired writer.
...
Persistence and fairness require identifying ``the same transition'' across configurations as goals are instantiated by reader substitutions. Following~\cite{lewis2026volitional}, we define goal identity and transition equivalence.
...
A transition system with transition equivalence $\sim$ is \temph{persistent} if: whenever a transition $t$ is enabled in configuration $c$, and $c = c_0 \rightarrow c_1 \rightarrow \cdots \rightarrow c_k = c'$ is a computation in which no $t' \sim t$ is taken, then some $t'' \sim t$ is enabled in $c'$.
\end{definition}

\begin{restatable}[cGLP Persistence]{lemma}{lemPersistence}
\label{lem:cglp-persistence}
cGLP is persistent.
\end{restatable}
...
%
Furthermore, if the success of a guard implies that $X?$ is bound to a ground term, then $X?$ as well as $X$ may occur multiple times in the clause. Groundness-implying guards include \verb|ground|, \verb|integer|, \verb|number|, \verb|string|, \verb|constant|, arithmetic comparisons (\verb|<|, \verb|>|, \verb|=<|, \verb|>=|, \verb|=:=|, \verb|=\=|), and ground equality (\verb|=?=|). However,  \verb|known| and \verb|compound| do not imply groundness.
\end{remark}
...
\end{verbatim}
\noindent The base case binds a plain number; the first addition clause guards that both operands are numbers and invokes the \verb|'_add'| body kernel; the fallback clause recursively evaluates subexpressions. The complete definitions of all system predicates and the full table of body kernels appear in \ref{appendix:guards-system}.
