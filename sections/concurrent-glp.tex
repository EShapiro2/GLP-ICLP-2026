...
GLP terms, unit goals, goals, and clauses are as in standard LP but defined over the variables in $\hat\calV$.
...
A network switch process routes messages between agents in simulation; in deployment, it is replaced by maGLP's Cold-call transaction (Section~\ref{sec:maglp}). The detailed walkthrough with code, figures, and a comprehensive scenario involving Alice, Bob, and Charlie appears in \appref{appendix:social-graph-walkthrough}{\arxivref}. The complete program---helper predicates, actor implementations, and boot variants---appears in \appref{appendix:social-graph-complete}{\arxivref}.
