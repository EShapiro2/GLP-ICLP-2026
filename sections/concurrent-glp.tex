...
A \temph{GLP renaming} is a substitution $\rho: \hat\calV \to \hat\calV$ such that for each $X \in \calV$: $X\rho \in \calV$ and $X?\rho = (X\rho)?$. Two GLP terms $T_1, T_2$ \temph{have a variable in common} if for some writer $X \in \calV$,  $T_1$ has an occurrence of $X$ or $X?$ and so does $T_2$. A GLP renaming $\sigma$ \temph{renames $T'$ apart from} $T$ if $T'\sigma$ and $T$ have no variable in common.
\end{definition}
...
\begin{restatable}{lemma}{lemPersistence}
