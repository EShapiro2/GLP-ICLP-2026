...
The social graph code presented in Section~\ref{sec:social-graph} runs unchanged in maGLP. The difference is in how inter-agent communication is realized: rather than a simulated network switch, the maGLP Network transaction directly transfers messages between agents' network streams.

When Alice executes the cold-call protocol to befriend Bob, her agent sends \verb|msg(bob, intro(alice, alice, Resp))| on the network output stream. The Network transaction (Definition~\ref{definition:maGLP}) transfers this message to Bob's network input stream, adding the reader \verb|Resp?| to Bob's resolvent. When Bob accepts, binding \verb|Resp| to \verb|accept(ch(FIn?, FOut))|, the Communicate transaction transfers this binding back to Alice.
...
The network switch simulation in Section~\ref{sec:social-graph} is thus a faithful model of maGLP's Network transaction, allowing single-process testing of multiagent protocols.
