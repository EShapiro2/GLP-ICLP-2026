...
\begin{itemize}
    ...
    \item \emph{Communicate transition}: $G' = G\hat\sigma?$ where $\hat\sigma? = \{X? := T\}$. Since $G$ satisfies SO, $X?$ occurs at most once in $G$. Replacing this single occurrence by $T$ (which satisfies SO by Definition~\ref{def:assignments}) preserves SO, provided $T$ shares no variables with the rest of $G$. By the proper run condition, variables in $T$ are fresh, so $G'$ satisfies SO.
\end{itemize}
\end{proof}
